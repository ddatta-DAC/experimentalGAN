...
class generator(nn.Module):
    ...
    def __init__(self, device, domain_dims, z_dim, encoder_hidden_dims=256, gumbel_T=0.25):
        ...
        self.encoder = nn.TransformerEncoderLayer(
            self.z_dim + self.num_dims,
            nhead=8,
            dim_feedforward=encoder_hidden_dims,
            dropout=0.05,
            activation='gelu'
        )
    ...
    def forward(
        ...
        pos = torch.eye(self.num_dims).reshape(
            [self.num_dims * self.num_dims]
        ).repeat(sample_size, 1).view(-1, self.num_dims, self.num_dims).to(self.device)
        # Shape : [ batch, self.num_dims , self.z_dim]
        z1 = z.repeat(1, self.num_dims).reshape([-1, self.num_dims, self.z_dim])

        # lstm input
        encoder_input = torch.cat([z1, pos], dim=-1)  # Shape [batch, self.num_dims , self.z_dim + self.num_dims ]
        # Output shape : (batch, seq, feature)
        encoder_op, _ = self.lstm(encoder_input)

        # Split the lstm_op
        x1 = torch.chunk(encoder_op, self.num_dims, dim=1)
